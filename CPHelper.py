...
class evaluate:
    ...
    def dataset_loader(IM_DIRs,image_format='jpg',label_format='tif',pred_format='tif',label_str='',pred_str=''):
        ...
        if not IM_DIR:
            IM_DIR = IM_DIRs
        for dir in IM_DIR:
            imgs1,lbls1,preds1 = evaluate.load_from_folders(dir,image_format=image_format,label_format=label_format,pred_format=pred_format,label_str=label_str,pred_str=pred_str)
            imgs += imgs1
            lbls += lbls1
            preds += preds1
